...
export function addCrawledWebsitesApi(app: express.Express, mongoClient: MongoClient) {
    ...
    app.get('/crawled-websites', async (request, response) => {
    ...
    app.get('/crawled-website', async (request, response) => {
        console.log(request.query);
        ...
        const crawledWebsites = getCrawledWebsitesCollection(mongoClient);
        const recordsCrawledWebsite = (await crawledWebsites.find({ url: validationResult.data.url }).project({ websiteRecordId: 1 }).toArray()).map(
            (id) => new ObjectId(id.websiteRecordId)
        );
